...
## Browsing complete histories

Discovery, admin market selection, portfolio holdings/orders/activity, and rankings have Previous/Next controls and direct page entry. Search, category filters, and sorting apply to the full dataset before pagination. Market charts page through 100 recorded trades per outcome; recent trades and order-book levels can also be paged. Changing a filter or portfolio tab starts at page one. Portfolio totals and reset eligibility always use all holdings, independently of the current page.

`GET /api/pages/:collection` returns `{ items, page, pageSize, total, pages }`. Collections are `markets`, `holdings`, `orders`, `open-orders`, `activity`, `trades`, and `leaderboard`. Requests accept `page` (starting at 1) and `pageSize` (1–100, default 24). Market filters are `search`, `category`, and `sort` (`newest`, `volume`, `closing`); `admin=true` includes hidden markets and requires an administrator. Trades require `marketId` and optionally `outcomeId`. Account collections always use the signed-in account. Out-of-range pages clamp to the last page; empty results report page one. Counts and rows share a database snapshot with deterministic tie-breakers. Changes between requests can shift page boundaries; live updates refresh the current page. Existing snapshot endpoints remain available for trading compatibility.
