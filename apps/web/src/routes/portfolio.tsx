...
import { Empty, Notice, SignIn, TableSkeleton } from '../components/ui';
...
function PortfolioPage() {
  ...
  if (!p) return <TableSkeleton rows={5} />;
  return (
    <>
      ...
      <div className="portfolio-stats">
        ...
        <div>
          ...
          <small>Lifetime settled result</small>
        </div>
      </div>
      {p.unpricedShares > 0 && (
        <Notice>
          {p.unpricedShares} shares have never traded and are excluded from the estimate.
        </Notice>
      )}
      ...
      <section className="reset-panel">
        <div>
          ...
          <p>
            {p.user.resetUsed
              ? 'Your one-time reset has been used.'
              : 'Clear your holdings and reset back to 10,000 play dollars. You get one reset.'}
          </p>
        </div>
        <button className="secondary" disabled={p.user.resetUsed} onClick={() => setReset(!reset)}>
          {p.user.resetUsed ? 'Reset used' : 'Reset account'}
        </button>
        {reset && !p.user.resetUsed && (
          <div className="reset-confirm">
            <p>This cancels your open orders. Settled results stay on the leaderboard.</p>
            <button
